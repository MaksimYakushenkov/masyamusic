...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false)
    if (!res.ok) setError(data.error || 'Ошибка регистрации')
    else router.push('/login')
  }

  const fields = [
    { label: 'ИМЯ ПОЛЬЗОВАТЕЛЯ', key: 'username', type: 'text', placeholder: 'musiclover' },
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {error && (
            <div
              ...
              style={{ background: 'rgba(255,65,0,0.1)', border: '1px solid rgba(255,65,0,0.3)', color: '#ff8c6b' }}
            >
              {error}
            </div>
          )}
          ...
          {fields.map(({ label, key, type, placeholder }) => (
            <div key={key}>
              <label className="block text-[10px] font-bold mb-2 tracking-widest" style={{ color: 'var(--text-muted)' }}>
                {label}
              </label>
              <input
                type={type}
